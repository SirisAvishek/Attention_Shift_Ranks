...
class AttentionLayer(Layer):
    ...
    def call(self, inputs):
        ...
        outputs = tf.reshape(outputs, (-1, self.config.TOP_K_DETECTION, self.config.BOTTLE_NECK_SIZE // self.config.NUM_ATTN_HEADS))

        return outputs

    def compute_output_shape(self, input_shape):
        return (None, self.config.TOP_K_DETECTION, self.config.BOTTLE_NECK_SIZE // self.config.NUM_ATTN_HEADS)
...
def attention_graph(theta_feat, phi_feat, g_feat, config):
    ...
    attn = Lambda(lambda z: tf.nn.softmax(z, axis=1))(attn)
